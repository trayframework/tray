...
class Wrapper extends FormDatagrid
{
    public $container;
    public $t;
    public $nl;
    ...
    {
        $this->FormDatagrid = $instance;
        # formatting: inserts a new line (\n)
        $this->nl = $this->FormDatagrid->_nl(1);
        # formatting: inserts a tab (\t)
        $this->t = $this->FormDatagrid->_t(1);
    }
    ...
    public function bootstrap3($element = '', $data = '')
    {
        ...
        $data['id'] = $this->FormDatagrid->make_id($data);
        ...
        if ($this->FormDatagrid->in_errors($data['name'])) {
        ...
        if ($this->FormDatagrid->is_not_empty($data['label'])) {
        ...
        if ($this->FormDatagrid->_check_required($data['name']) && $this->FormDatagrid->is_not_empty($data['label'])) {
            if ($data['type'] != 'checkbox' && $data['type'] != 'radio') {
                $return .= $this->FormDatagrid->required_indicator;
            }
        }
    ...
    public function bootstrap4($element = '', $data = '')
    {
        ...
        if ($this->FormDatagrid->type_is_checkbox($data))
        {
            # input is a checkbox or radio
            # don't print the label if we're printing an array
            if (! $this->FormDatagrid->is_array($data['value']))
            {
        ...
        # see if we're in a checkbox array...
        if ($this->FormDatagrid->is_array($data['name'])) {
            # we are. we don't want to color each checkbox label if there's an error - we only want to color the main label for the group
            $return .= $this->t.'<label class="col-form-label col-lg-2 col-12" for="'.$data['id'].'">'.$data['label'].$this->nl;
        } else {
        ...
        if (! $this->FormDatagrid->type_is_checkbox($data)) {
            $return .= '<div class="col-xl-6 col-12">'.$this->t.$element.'</div>';
        }
        ...
        if (!empty($data['inline']))
        {
            # help-block text
            # if the text is surrounded by square brackets, show only on form error
            if ($this->FormDatagrid->is_in_brackets($data['inline'])) {
                if ($this->FormDatagrid->in_errors($data['name'])) {
                    # trim the brackets and show on error
        ...
        # checkbox/radio: add the label text and close the label tag
        if ($this->FormDatagrid->is_not_empty($data['label']) && $this->FormDatagrid->type_is_checkbox($data))
        {
            # add label text
            $return .= ' '.$data['label'];

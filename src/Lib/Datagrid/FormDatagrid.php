...
use Tray\Lib\Datagrid\Wrapper;
class FormDatagrid
{
    # each of these public properties acts as a 'preference' for FormDatagrid
    # and can be defined at instantiation. see documentation for more info.
    protected $_debuglist = [];
    ...
    # suppress FormDatagrid's validation error messages and only display your own
    ...
    # default wrapper types which FormDatagrid supports
    ...
    function __construct($wrapper = '')
    {
        ...
        if (!$this->wrapper || in_array($this->wrapper, $this->default_wrapper_types)) {
            ...
            $this->controls = Wrapper::bootstrap4_css();
        }
    }
            ...
            if (isset($_POST['csrf_token']))
            {
                ...
                if (hash_equals($_SESSION['token'], $_POST['csrf_token']))
                {
                    ...
                    if (time() >= $_SESSION['token-expires']) {
                        $this->add_to_errors('Session has timed out.');
                        return false;
                    }
                  } else {
                    $this->add_to_errors('Token mismatch.');
                    return false;
                }
            }
    ...
    protected function _clean_value($str = '', $html = false)
    {
        ...
        # FormDatagrid can also use HTMLPurifier
        # just download the HTMLPurifier class and drop it in at the top of this script. FormDatagrid will do the rest.
        ...
        foreach ($data['options'] as $key => $value) {
            ...
            if (is_array($value)) {
                ...
                foreach ($value as $value => $label) {
                   ...
                   // Debug($data['selected']); Debug($value);exit;
                    ...
                    elseif ($data['selected'] == $value || (is_array($data['selected']) && in_array($value, $data['selected']))) {
                        $return .= $this->_t(2) . '<option value="' . $value . '" selected="selected">' . $label . 'xxxx</option>';
                    }
    ...
    {
        # suppress FormDatagrid's default validation error messages and only show user-defined messages
